#importação manual dos módulos de codificação e simulação do modelo BD do phylodeep
...
def encode_into_summary_statistics(tree_input, sampling_proba):
    ...
    name_tree(tree)
    max_depth_raw = sumstats.add_depth_and_get_max(tree)
    # Ensure the array-sized parameter includes the deepest index; depths are 0-based
    max_depth = int(max_depth_raw) + 1 if int(max_depth_raw) >= 0 else 1
    ...
    # Skip trees with insufficient events for robust LTT regression/coordinates
    try:
        if ltt_plot_matrix is None:
            return None, rescale_factor
        if hasattr(ltt_plot_matrix, 'shape') and ltt_plot_matrix.shape[0] < 2:
            return None, rescale_factor
        if isinstance(ltt_plot_matrix, np.ndarray) and ltt_plot_matrix.size == 0:
            return None, rescale_factor
        ...
        # e.g., regression input empty; skip this tree gracefully
        return None, rescale_factor
    except Exception:
        # any other unexpected failure in LTT computations
        return None, rescale_factor
...
def encode_into_most_recent(tree_input, sampling_proba):
    ...
    # local copy of input tree
    #tree = tree_input.copy()
    tree = Tree("testando_formato_before.nw", format=1)
