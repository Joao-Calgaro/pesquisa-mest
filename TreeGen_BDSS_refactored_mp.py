...
if __name__ == "__main__":
    ...
    simulacoes_com_erro = [r["error"] for r in results if "error" in r]

    tree_arrays = [r["tree_array"] for r in results_valid]
    rescale_factors = [r["rescale_factor"] for r in results_valid]
    node_ordering = [r["internal_nodes_order"] for r in results_valid]
    nodes_ancestor = [r["ancestor"] for r in results_valid]
    mutation_positions = [r["mutation_position"] for r in results_valid]
    number_of_mutantes = [r["number_of_mutantes"] for r in results_valid]
    time_of_surgimento_mutacao = [r["time_of_surgimento_mutacao"] for r in results_valid]
    total_time_of_simulation = [r["total_time_of_simulation"] for r in results_valid]

    save_data = {
        'tree_data': np.array(tree_arrays),
        'rescale_factor': np.array(rescale_factors),
        'internal_nodes_order': np.array(node_ordering, dtype=object),
        'ancestor': np.array(nodes_ancestor, dtype=object),
        'mutation_positions': np.array(mutation_positions, dtype=object),
        'number_of_mutantes': np.array(number_of_mutantes),
        'time_of_surgimento_mutacao': np.array(time_of_surgimento_mutacao, dtype=object),
        'total_time_of_simulation': np.array(total_time_of_simulation),
        'simulacoes_com_erro': np.array(simulacoes_com_erro, dtype=object),
        'tabela_parametros': design.values
    }

    np.savez_compressed("mutation_deep_learning_mp_10000.npz", **save_data)
